...
const LINE1_WORDS = ["¿Buscas", "algo", "único..."];
const LINE2_WORDS = ["...o", "quieres", "venderlo?"];
...
export const MarketplaceHookScene: React.FC = () => {
  const frame = useCurrentFrame();
      ...
      >
        <div style={{ display: "flex", gap: Math.round(fontSize * 0.32) }}>
          {LINE1_WORDS.map((word, i) => (
            <RevealWord key={i} word={word} startFrame={6 + i * 7} fontSize={fontSize} />
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: Math.round(fontSize * 0.32) }}>
          {LINE2_WORDS.map((word, i) => (
            <RevealWord
              ...
              startFrame={34 + i * 7}
              fontSize={fontSize}
              color={i === LINE2_WORDS.length - 1 ? "#FE9200" : "#fff"}
            />
          ))}
        </div>
      </div>
