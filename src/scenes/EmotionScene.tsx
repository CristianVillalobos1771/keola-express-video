...
export const EmotionScene: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps, width, height } = useVideoConfig();

  // Quick punch-in, hold, fade out
  ...
  // Main text: punch (scale 1.35 → 1.0)
  ...
  // Sub-words stagger
  const subWords = ["Rápido.", "Caliente.", "Sin salir de casa."];
  const subStart = 30;
  const subStagger = 9;
  ...
  return (
    ...
    >
      {/* Confetti (45 particles) */}
      {Array.from({ length: 45 }).map((_, i) => (
        <Particle key={i} seed={`cfti-${i}`} delay={i * 2} />
      ))}
